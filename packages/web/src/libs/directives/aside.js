...
function handleScroll() {
  ...
  if (asideBottom < document.documentElement.clientHeight) {
    // aside高度还不足整个可见区域
    aside.style.cssText = `position: sticky;top: ${asideTop}px`;
    return;
  }
  if (windowScrollBottom > footerTop) {
    // 阶段3 sticky
    phase.value = 3;
  } else if (windowScrollBottom > asideBottom && windowScrollBottom < footerTop) {
    // 阶段2 fixed
    phase.value = 2;
  } else if (windowScrollBottom < asideBottom) {
    // 阶段1 relative
    phase.value = 1;
  }
