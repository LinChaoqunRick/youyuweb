      ...
      <div class="content-wrapper">
        ...
        <div class="todo-list" v-if="todoList?.length">
          ...
          <div class="todo-items">
            <div class="todo-item" v-for="item in todoList" @click="onClickTodo">
              <div class="item-type" :class="[`item-type-${item.type}`]">{{ item.typeName.slice(0, 1) }}</div>
              <div class="item-title text-limit" :title="item.title">{{ item.title }}</div>
            </div>
          </div>
        </div>
      </div>
