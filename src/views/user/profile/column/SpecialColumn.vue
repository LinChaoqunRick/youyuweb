<template>
  <div class="special-column">
    <div class="column-header" v-if="isOwn">
      <span>最多可创建 {{ maxNum }} 个专栏，当前 <span
        :class="{'active':!!columnList.length}">{{ columnList.length }}</span>/{{ maxNum }} </span>
    ...
    <ContentData url="getColumnList" :params="{userId: user.id}" v-slot="{data}" ref="ContentDataRef">
      <div class="column-list" v-if="data && data.length">
        <ColumnItem @deleteSuccess="deleteSuccess" @updateSuccess="updateSuccess" v-for="item in data" :data="item"/>
      </div>
    </ContentData>
  </div>
</template>
