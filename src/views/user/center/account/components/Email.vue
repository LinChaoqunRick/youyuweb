      ...
      <a-form-item label=" " name="code">
        <a-input v-model:value="formValidate.code" :maxlength="6" size="large" placeholder="验证码">
          <template v-slot:suffix>
            <a-button type="link" class="send-code-btn" :disabled="sendBtnProps.disabled"
                      :loading="sendBtnProps.loading" @click="onSendCode">
              {{sendBtnProps.text}}
            </a-button>
          </template>
        </a-input>
...
<script setup lang="ts">
  ...
  function onSendCode() {
    ...
    formRef.value.validateFields('email').then(res => {
      dispatch('sendEmailCode', {target: formValidate.email, repeat: true}).then(res => {
