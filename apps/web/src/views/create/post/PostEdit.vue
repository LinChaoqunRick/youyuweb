<template>
  <a-spin :spinning="spinning" tip="加载中...">
    <PostEditor :form-validate="formValidate" @handle-submit="handleSubmit" />
  </a-spin>
</template>
...
const initData = () => {
  dispatch('getPostEditDetail', { postId: route.query.postId })
    .then(res => {
...
onBeforeRouteLeave((to, from) => {
  if (isSave.value) {
    ...
    const answer = window.confirm('确定离开此页面？请确保您的变更已被保存!');
    // 取消导航并停留在同一页面上
