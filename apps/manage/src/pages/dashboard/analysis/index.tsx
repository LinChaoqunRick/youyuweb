...
function Analysis() {
  ...
  const [visitType, setVisitType] = useState<'daily' | 'monthly'>('daily');
  ...
  const generateContentChartOptions = (data: OverviewData | null): EChartsOption => {
    ...
    return getBarOptions({
      ...
      series: [
        {
          name: '内容数量',
          data: [data.postNumber, data.momentNumber, data.noteNumber, data.chapterNumber, data.albumNumber],
        },
      ],
    });
  };
  ...
  const generateInteractionChartOptions = (data: OverviewData | null): EChartsOption => {
    ...
    return getBarOptions({
      ...
      series: [
        {
          name: '互动数量',
          data: [data.postCommentNumber, data.momentCommentNumber, data.messageNumber],
        },
      ],
    });
  };
  ...
  useEffect(() => {
    Promise.all([http.get<AnalysisData>(GET_ANALYSIS_DATA), http.get<OverviewData>(GET_OVERVIEW_DATA)])
  ...
  return (
    <div className="analysis-container">
      <Row gutter={[12, 12]}>
        <Col xs={24} lg={14}>
          <Card
            ...
            style={{ height: '100%' }}
            extra={
              <Radio.Group value={visitType} onChange={e => setVisitType(e.target.value)} size="small">
                <Radio.Button value="daily">今日</Radio.Button>
            ...
            <Row gutter={[12, 12]}>
              <Col xs={24} lg={12}>
                <div style={{ height: 240 }}>
                  <ReactEChart
                    options={visitType === 'daily' ? dailyRegionOptions : monthlyRegionOptions}
                    spinning={loading}
                  />
                </div>
              </Col>
              <Col xs={24} lg={12}>
                <div style={{ height: 240 }}>
                  <ReactEChart
                    options={visitType === 'daily' ? dailyVisitsOptions : monthlyVisitsOptions}
                    spinning={loading}
                  />
                </div>
              </Col>
      ...
      <Row gutter={[12, 12]}>
        <Col xs={24} lg={8}>
          <Card title="游客地域分布" style={{ height: '100%' }}>
            <div style={{ height: 240 }}>
        ...
        <Col xs={24} lg={16}>
          <Card title="月评论趋势" style={{ height: '100%' }}>
            <div style={{ height: 240 }}>
